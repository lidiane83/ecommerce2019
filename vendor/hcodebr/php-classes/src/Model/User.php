...
class User extends Model{
    ...
    $sql = new Sql();
    $results =   $sql->select("CALL sp_usersupdate_save(:iduser,:desperson, :deslogin, :despassword, :desemail,:nrphone,:inadmin)",array(":iduser"=>$this->getiduser(),":desperson"=>$this->getdesperson(),":deslogin"=>$this->getdeslogin(),":despassword"=>$this->getdespassword(),":desemail"=>$this->getdesemail(),":nrphone"=>$this->getnrphone(),":inadmin"=>$this->getinadmin()));
    $this->setData($results[0]);
    }
    public function delete(){
        $sql = new Sql();
        $results = $sql->delete("CALL sp_usersupdate_save(:iduser)",array(":iduser"=> $this->getiduser()));
...
{
    $sql=new Sql();
    $sql->query("UPDATE tb_passwordrecoveries SET dtrecovery = Now() where idrecovery=:idrecovery", array(":idrecovery"=>$idrecovery));
}
